...
const CampDetail = ({ user, loading, loadUser }) => {
	...
	const handleImport = async (e) => {
		const res = await axios.post(
			`http://localhost:5000/api/objects/import/${hotelID}/${kampID}`,
			{
	...
	const handleSubmit = async (e) => {
		try {
			...
			await axios.post("http://localhost:5000/api/objects", newObj);
	...
	const fetchObj = async () => {
		let res = await axios.get(`http://localhost:5000/api/objects/${kampID}`, {
			params: {
				pageNumber: page,
			},
		});
